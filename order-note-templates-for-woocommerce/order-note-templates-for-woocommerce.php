<?php

/**
 * Plugin Name: Pro Web Design Order Note Templates for WooCommerce
 * Plugin URI:  https://wordpress.org/plugins/pro-web-design-order-note-templates-for-woocommerce/
 * Description: Save and reuse order note templates in WooCommerce admin. Works with HPOS and WooCommerce Subscriptions.
 * Version:     1.2.5
 * Author:      Pro Technologies Limited
 * Author URI:  https://pro-webdesign.co.uk
 * Text Domain: pro-web-design-order-note-templates-for-woocommerce
 * Domain Path: /languages
 * Requires at least: 5.8
 * Requires PHP: 7.4
 * Requires Plugins: woocommerce
 * WC requires at least: 6.0
 * WC tested up to: 10.8
 * License:     GPLv2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 */
defined( 'ABSPATH' ) || exit;
// Auto-deactivate free version when Pro is activated
if ( function_exists( 'wc_ont_fs' ) ) {
    wc_ont_fs()->set_basename( false, __FILE__ );
} else {
    /**
     * DO NOT REMOVE THIS IF. IT IS ESSENTIAL FOR THE
     * `function_exists` CALL ABOVE TO PROPERLY WORK.
     */
    if ( !function_exists( 'wc_ont_fs' ) ) {
        define( 'WC_ONT_VERSION', '1.2.5' );
        define( 'WC_ONT_FILE', __FILE__ );
        define( 'WC_ONT_DIR', plugin_dir_path( __FILE__ ) );
        define( 'WC_ONT_URL', plugin_dir_url( __FILE__ ) );
        /* -------------------------------------------------------------------------
         * Freemius SDK
         * ---------------------------------------------------------------------- */
        function wc_ont_fs() {
            global $wc_ont_fs;
            if ( !isset( $wc_ont_fs ) ) {
                require_once dirname( __FILE__ ) . '/vendor/freemius/start.php';
                $wc_ont_fs = fs_dynamic_init( array(
                    'id'               => '36694',
                    'slug'             => 'order-note-templates-for-woocommerce',
                    'type'             => 'plugin',
                    'public_key'       => 'pk_cf33727630f61efd2baf1a4b67938',
                    'is_premium'       => false,
                    'premium_suffix'   => 'Professional',
                    'has_addons'       => false,
                    'has_paid_plans'   => true,
                    'is_org_compliant' => true,
                    'trial'            => array(
                        'days'               => 14,
                        'is_require_payment' => false,
                    ),
                    'menu'             => array(
                        'support' => false,
                    ),
                    'is_live'          => true,
                ) );
            }
            return $wc_ont_fs;
        }

        wc_ont_fs();
        do_action( 'wc_ont_fs_loaded' );
        /* -------------------------------------------------------------------------
         * HPOS compatibility
         * ---------------------------------------------------------------------- */
        add_action( 'before_woocommerce_init', function () {
            if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
                \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', __FILE__, true );
            }
        } );
        /* -------------------------------------------------------------------------
         * Helpers
         * ---------------------------------------------------------------------- */
        function wc_ont_is_pro() {
            return function_exists( 'wc_ont_fs' ) && wc_ont_fs()->can_use_premium_code__premium_only();
        }

        /* -------------------------------------------------------------------------
         * Activation
         * ---------------------------------------------------------------------- */
        register_activation_hook( __FILE__, 'wc_ont_activate' );
        function wc_ont_activate() {
            wc_ont_create_table();
            wc_ont_insert_defaults();
        }

        /**
         * Create or upgrade the templates table.
         *
         * The `category` and `pdf_attachment` columns are Pro-only functionality,
         * so the free build must not create them at all — only the Pro build (where
         * wc_ont_is_pro() is true) adds them. dbDelta() only ever adds missing
         * columns, never drops existing ones, so a site that upgrades from Free to
         * Pro gets them added on the fly, and a site that reverts to Free simply
         * stops using — but does not lose — them. Note: no "IF NOT EXISTS" in the
         * SQL below; dbDelta() parses the table name with a regex and would read
         * "IF" as the table name.
         */
        function wc_ont_create_table() {
            global $wpdb;
            $table   = $wpdb->prefix . 'order_note_templates';
            $charset = $wpdb->get_charset_collate();

            $pro_columns = '';
            $pro_keys    = '';
            if ( wc_ont_is_pro() ) {
                $pro_columns = "\n                category varchar(100) NOT NULL DEFAULT '',\n                pdf_attachment varchar(500) NOT NULL DEFAULT '',";
                $pro_keys    = "\n                KEY category (category),";
            }

            // dbDelta is picky: one field per line, two spaces after PRIMARY KEY.
            $sql = "CREATE TABLE {$table} (\n                id bigint(20) unsigned NOT NULL AUTO_INCREMENT,\n                title varchar(200) NOT NULL,\n                note_text text NOT NULL,\n                note_type varchar(20) NOT NULL DEFAULT 'customer',{$pro_columns}\n                sort_order int(11) NOT NULL DEFAULT 0,\n                created_at datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,\n                PRIMARY KEY  (id),\n                KEY note_type (note_type),{$pro_keys}\n                KEY sort_order (sort_order)\n            ) {$charset};";
            require_once ABSPATH . 'wp-admin/includes/upgrade.php';
            dbDelta( $sql );
            update_option( 'wc_ont_db_version', WC_ONT_VERSION );
        }

        /**
         * True when the given column exists on the templates table.
         */
        function wc_ont_column_exists(  $column  ) {
            global $wpdb;
            $table = $wpdb->prefix . 'order_note_templates';
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter
            $found = $wpdb->get_results( $wpdb->prepare( "SHOW COLUMNS FROM {$table} LIKE %s", $column ) );
            return !empty( $found );
        }

        function wc_ont_insert_defaults() {
            global $wpdb;
            $table = esc_sql( $wpdb->prefix . 'order_note_templates' );
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            if ( $wpdb->get_var( "SELECT COUNT(*) FROM {$table}" ) > 0 ) {
                return;
            }
            $defaults = array(array(
                'Order received',
                'Your order #{order_id} has been received and is being processed.',
                'customer',
                10
            ), array(
                'Order shipped',
                'Your order #{order_id} has been shipped. Expected delivery: 3-5 days.',
                'customer',
                20
            ), array(
                '[Internal] Awaiting stock',
                'Waiting for warehouse stock confirmation.',
                'internal',
                10
            ));
            foreach ( $defaults as $d ) {
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- seeding a custom table on activation; no cache to prime.
                $wpdb->insert( $wpdb->prefix . 'order_note_templates', array(
                    'title'      => $d[0],
                    'note_text'  => $d[1],
                    'note_type'  => $d[2],
                    'sort_order' => $d[3],
                ) );
            }
        }

        /* -------------------------------------------------------------------------
         * Boot
         * ---------------------------------------------------------------------- */
        add_action( 'plugins_loaded', 'wc_ont_init' );
        function wc_ont_init() {
            if ( !class_exists( 'WooCommerce' ) ) {
                add_action( 'admin_notices', function () {
                    echo '<div class="notice notice-error"><p><strong>Order Note Templates for WooCommerce</strong>: WooCommerce must be active.</p></div>';
                } );
                return;
            }
            /*
             * Run the schema migration synchronously, before anything that
             * writes to the table is loaded. Do not defer this to a hook —
             * we are already inside plugins_loaded here.
             *
             * The category/pdf_attachment columns are Pro-only, so a free install
             * must never be made to "self-heal" them back into existence — only
             * check for them (and add them) when running the Pro build.
             */
            $needs_schema_upgrade = get_option( 'wc_ont_db_version' ) !== WC_ONT_VERSION;
            if ( wc_ont_is_pro() ) {
                $needs_schema_upgrade = $needs_schema_upgrade || !wc_ont_column_exists( 'category' ) || !wc_ont_column_exists( 'pdf_attachment' );
            }
            if ( $needs_schema_upgrade ) {
                wc_ont_create_table();
            }
            require_once WC_ONT_DIR . 'includes/class-admin-page.php';
            require_once WC_ONT_DIR . 'includes/class-order-meta-box.php';
            require_once WC_ONT_DIR . 'includes/class-ajax.php';
        }

    }
    // end if ( ! function_exists )
}
// end else